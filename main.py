...
def evaluate(model, valid_X, attack_path, output_file):
    if "OneClassSVM" in str(type(model)):
        ...
        print(f"Attack Path: {attack_path} - Detection Accuracy: {res:.4f} | False Positive Rate: {1 - res:.4f}")
        ...
        print(f"Attack Path: {attack_path} - Loss: {res[0]:.4f} - Accuracy: {res[1]:.4f}")
        with open(output_file, "a", encoding="utf-8") as file:
        ...
        for th in threshold:
            ...
            print(f"Threshold: {th:.2f} - Anomaly Detection Rate: {res_:.4f}")
            with open(output_file, "a", encoding="utf-8") as file:
...
one_hot_encoder = ColumnTransformer(
    transformers=[
        ('cat', preprocessing.OneHotEncoder(), [1, 2, 3])  # Columns 1, 2, and 3 are categorical
    ],
...
def read_kdd_dataset(path):
    ...
    if hasattr(label_encoder_1, 'classes_') == False:
        ...
        dataset_features = one_hot_encoder.fit_transform(dataset[:, :-2]) 
    else:
        ...
        dataset_features = one_hot_encoder.transform(dataset[:, :-2])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--normal_path', default=r'D:\VS coding\Btech project\zero-day-detection\DataFiles\CIC\biflow_Monday-WorkingHours_Fixed.csv')
    parser.add_argument('--attack_paths', default=r'D:\VS coding\Btech project\zero-day-detection\DataFiles\CIC')
...
with open(output_file, "a", encoding="utf-8") as file:
    file.write(f"Training Accuracy: {res1[1]:.4f} - Validation Accuracy: {res2[1]:.4f}\n")
    file.write(f"Training Loss: {res1[0]:.4f} - Validation Loss: {res2[0]:.4f}\n")
    
    print(f"Training Accuracy: {res1[1]:.4f}")
    print(f"Validation Accuracy: {res2[1]:.4f}")
    print(f"Training Loss: {res1[0]:.4f}")
    print(f"Validation Loss: {res2[0]:.4f}")


import matplotlib.pyplot as plt

# Add this after the evaluation loop, or in a new section after all evaluations.
def plot_attack_detection_performance(output_file):
    # For example, after gathering detection results, you could plot the anomaly detection rate
    thresholds = []
    detection_rates = []
    with open(output_file, 'r', encoding='utf-8') as file:  # Specify utf-8 encoding here
        lines = file.readlines()
        for line in lines:
            if 'Threshold' in line:
                threshold_value = float(line.split(":")[1].strip())
                detection_rate = float(line.split("Anomaly Detection Rate:")[1].strip())
                thresholds.append(threshold_value)
                detection_rates.append(detection_rate)

    # Plotting the performance over thresholds
    plt.plot(thresholds, detection_rates, label='Anomaly Detection Rate')
    plt.xlabel('Threshold')
    plt.ylabel('Detection Rate')
    plt.title('Zero-Day Attack Detection Performance')
    plt.legend()
    plt.show()


# Call the plot function after all evaluations are done.
import joblib

# Assuming 'wrapper.model' is your trained One-Class SVM model
if args.model == 'svm':
    # Save the One-Class SVM model to a file
    model_path = 'oneclass_svm_model.joblib'
    joblib.dump(wrapper.model, model_path)
